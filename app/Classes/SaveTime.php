...
class SaveTime
{
	public function time($user, $firstDay, $day, $numOfDays, $fromDatabase, $getLastDay)
    {
		...
		function getHours($day, $hour, $user) {
			...
			return array('array' => $array, 'minutes_left' => array('start' => $beginOriginal->format('Y-m-d H:i'), 'close' => $end->format('Y-m-d H:i'), 'max_available_minutes' => $minutes_open, 'company_id' => $user->company_id, 'employer_id' => $user->id));
		}
